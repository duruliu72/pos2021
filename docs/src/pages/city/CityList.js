...
import {getCities } from "../../store/cityReducer";
...
const CityList = () => {
    ...
    const showForEdit=(item) => {
        history.push("/cities/"+ item.id);
    }
    const handleDelete=(item) => {
        if(window.confirm("Do you really want to Delete?")) {
            axios.delete("http://localhost:8080/api/cities/"+item.id)
            .then(({data:res}) => {
                 dispatch({
                     type:"DELETE_CITY",
                     payload: res,
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-sm-12">
                        <h4 className="pull-left page-title">City LIST</h4>
                        <ol className="breadcrumb pull-right">
                            <li><Link to="/">Beauty4Me</Link></li>
                            <li><Link to="/cities/new">Create City</Link></li>
                        </ol>
                    </div>
                </div>
                <div className="row">
