...
import {getCompany } from "../../store/companyReducer";
const UpdateCompany = () => {
    const dispatch = useDispatch();
    const imageUrl=useRef();
    let { companyid } = useParams();
    const findCompany =useSelector((state) => {
        return getCompany(state,companyid);
    });
    ...
    let handleSubmit=(e) => {
        ...
        axios.put(apiUrl + "/companies/"+company.id, fd)
        .then(({data:res}) => {
             dispatch({
                 type:"UPDATE_COMPANY",
                 payload: res,
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-sm-12">
                        <h4 className="pull-left page-title">Update Company</h4>
                        <ol className="breadcrumb pull-right">
                            <li><Link to="/">Beauty4Me</Link></li>
                            <li><Link to="/companies">Companies</Link></li>
                            <li className="active">Update Company</li>
                        </ol>
                    </div>
                </div>  
