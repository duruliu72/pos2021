...
import {getCountries } from "../../store/countryReducer";
...
const CountryList = () => {
    ...
    const showForEdit=(item) => {
        history.push("/countries/"+ item.id);
    }
    const handleDelete=(item) => {
        if(window.confirm("Do you really want to Delete?")) {
            axios.delete("http://localhost:8080/api/countries/"+item.id)
            .then(({data:res}) => {
                 dispatch({
                     type:"DELETE_COUNTRY",
                     payload: res,
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-sm-12">
                        <h4 className="pull-left page-title">Country List</h4>
                        <ol className="breadcrumb pull-right">
                            <li><Link to="/">Beauty4Me</Link></li>
                            <li><Link to="/countries/new">Create Country</Link></li>
                        </ol>
                    </div>
                </div>
                <div className="row">
