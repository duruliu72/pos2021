...
import {getGenders } from "../../store/genderReducer";
...
const GenderList = () => {
    ...
    const showForEdit=(item) => {
        history.push("/genders/"+ item.id);
    }
    const handleDelete=(item) => {
        if(window.confirm("Do you really want to Delete?")) {
            axios.delete("http://localhost:8080/api/genders/"+item.id)
            .then(({data:res}) => {
                 dispatch({
                     type:"DELETE_GENDER",
                     payload: res,
                 });
                 toast("Gender Deleted.");
             })
             .catch(ex => {
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-sm-12">
                        <h4 className="pull-left page-title">Gender LIST</h4>
                        <ol className="breadcrumb pull-right">
                            <li><Link to="/">Beauty4Me</Link></li>
                            <li><Link to="/genders/new">Create Gender</Link></li>
                        </ol>
                    </div>
                </div>
                <div className="row">
